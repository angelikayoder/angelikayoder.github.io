/********************************************************
 *
 * Custom Javascript code for strapped Bootstrap theme
 * 
 *
 *******************************************************/
var jPM = {},
...
(function($) {
  $.extend($.fn, {
    ...
    themeCustom: function(refresh) {
      ...
      // ----------------------------------------------------------------
      // color switch - demo only
      // ----------------------------------------------------------------
      var defaultcolor = $('body').data('color-scheme') || 'green';
      var colorSchemes = context.find('.theme-colors a');
      colorSchemes.removeClass('active');
      colorSchemes.filter('.' + defaultcolor).addClass('active');
      colorSchemes.click(function() {
        var $this = $(this);
        var c = $this.attr('href').replace('#', '');
        var cacheBuster = 3 * Math.floor(Math.random() * 6);
        $('.theme-colors a').removeClass('active');
        $('.theme-colors a.' + c).addClass('active');

        if (c !== defaultcolor) {
          context.find('#color-scheme').attr('href', 'assets/css/color-' + c + '.css?x=' + cacheBuster);
        } else {
          context.find('#color-scheme').attr('href', '#');
        }
      });
    ...
    themePluginsDefault: function(context) {
      ...
      return {
          ...
          if ($sliderRevolutions.length > 0) {
            var themePluginSliderRevolutionInit = function() {
              ...
              $sliderRevolutions.each(function() {
                ...
                if (customInit) {
                  sliderRevEl.addClass('custom-init').trigger("strapped:sliderRev:customInit", [sliderRevSettings]);
                  return;
                } else {
                  sliderRevEl.hide();
                  var sliderAPI = sliderRevEl.addClass('standard-init').show().revolution(sliderRevSettings);
                  sliderRevEl.trigger("strapped:sliderRev:standardInit", [sliderRevSettings]);
        ...
        themePluginBlazy: function() {
          ...
          //
          // From strapped 3.0.9 data-bg-img is now loaded by BLazy too
          // ----------------------------------------------------------------          
